...
A implementação utilizando arrays e as chamadas de seus métodos padroniza a nomenclatura para operações de uma lista. Porém o uso unico dos métodos oculta alguns custos que existem nas operações. Observe o uso dos métodos splice na remoção (removeAt) e na inserção (insert) de elementos. Este método faz a realocação dos índices o que se mostra uma operação cara computacionalmente. 

    Faça esta implementação da lista sem o uso dos métodos do array para destacar as iterações. 


## Implementação com Classes e Arrays

Da mesma forma que estudamos as estruturas de dados até então usando funções manipulando arrays, também criamos classes para encapsular o comportamento e os dados. Na implementação a seguir temos o uso dos arrays e a implementação do comportamento das listas nos métodos da classe ArrayList.
